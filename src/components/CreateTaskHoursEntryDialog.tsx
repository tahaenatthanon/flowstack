...
import { useCreateTaskHoursEntry, useUpdateTaskHoursEntry, useTasks } from '@/hooks/useProjectData';
...
export interface CreateTaskHoursEntryDialogProps {
  onSuccess?: () => void;
  open?: boolean;
...
const CreateTaskHoursEntryDialog = ({
  onSuccess,
  open: openProp,
  onOpenChange,
  hideTrigger = false,
  ...
  className,
  editEntry,
}: CreateTaskHoursEntryDialogProps) => {
    ...
    <DialogContent className="w-full sm:max-w-2xl sm:max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle className="font-heading text-xl flex items-center gap-2">
          ...
          {isEditMode ? 'แก้ไขบันทึกเวลา' : 'บันทึกชั่วโมงงาน (บันทึกชั่วโมง)'}
        </DialogTitle>
      </DialogHeader>
...
export default CreateTaskHoursEntryDialog;
