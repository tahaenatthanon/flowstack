...
import { useDeleteTimesheetEntry, useUpdateTimesheetEntry } from '@/hooks/useProjectData';
...
const TimesheetTable = ({ entries, tasks, projectId }: TimesheetTableProps) => {
  ...
  const [editing, setEditing] = useState<DbTimesheetEntry | null>(null);
  ...
  const handleEdit = (entry: DbTimesheetEntry) => {
    setEditing(entry);
    setOpen(true);
  };

  const handleDelete = async (entry: DbTimesheetEntry) => {
  ...
  return (
    <div className="bg-card rounded-xl border p-3 sm:p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold font-heading">Timesheet</h3>
      ...
      {entries.length === 0 ? (
        <p className="text-sm text-muted-foreground py-4 text-center">ไม่มีข้อมูล Timesheet</p>
      ) : (
