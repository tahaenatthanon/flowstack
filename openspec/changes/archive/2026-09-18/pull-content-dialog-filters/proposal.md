...
หน้าต่าง "ดึงคอนเทนต์" (`PullFromContentDialog`) ในหน้าแคมเปญอีเมล มีแค่ช่องค้นหาด้วยชื่อเรื่องเท่านั้น ไม่มีตัวกรองมิติอื่นเลย และที่สำคัญกว่านั้นคือ backend (`GET /content-items.php`) ไม่กรองสถานะใดๆ — ดึงข้อมูล**ทุกสถานะ**มาแสดงรวมกันหมด ทั้ง `draft`, `revision`, `pending_approval`, `rejected` ปนกับ `approved`/`published`
...
- เพิ่มตัวกรอง 3 มิติในหน้าต่าง "ดึงคอนเทนต์" เรียงตามลำดับ: **ประเภท** (article/video/image) → **สถานะ** (draft/pending_approval/approved/revision/rejected/published) → **แพลตฟอร์ม** (facebook/lineoa/...)
...
### New Capabilities
- `campaign-content-picker-filters`: กำหนดว่าหน้าต่าง "ดึงคอนเทนต์" ในหน้าแคมเปญอีเมลกรองรายการคอนเทนต์ตามประเภท/สถานะ/แพลตฟอร์มอย่างไร รวมถึงค่าเริ่มต้นของตัวกรองสถานะที่ต้องปลอดภัยต่อการส่งแคมเปญจริง
