...
ทุกจุด UI ที่ยังแสดงแพลตฟอร์มของ content item (`ContentCardDialog`, `ContentDetailView`, และ `PullFromContentDialog`) SHALL parse ค่าแพลตฟอร์ม (ไม่ว่าจะมาจาก `platforms` แบบ JSON array หรือ `platform` แบบ comma-joined string) เป็นรายการแพลตฟอร์มแยกกัน แล้วแสดงแต่ละแพลตฟอร์มพร้อมไอคอน/สีของแพลตฟอร์มนั้นๆ โดยใช้ `PlatformBadgeList` เป็น component กลางร่วมกัน ห้ามนำค่าดิบทั้งก้อนไป lookup เป็นแพลตฟอร์มเดียว และห้ามเขียน logic parse/render แพลตฟอร์มแยกเอง
...
- **THEN** ทั้ง `ContentCardDialog`, `ContentDetailView`, และ `PullFromContentDialog` แสดงไอคอน/สีของแพลตฟอร์มนั้นถูกต้อง เหมือนพฤติกรรมเดิม
