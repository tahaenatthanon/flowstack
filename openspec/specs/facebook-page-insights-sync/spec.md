# facebook-page-insights-sync Specification

## Purpose

เก็บ page insights รายวันของเพจ Facebook (22 metric) ลงตาราง `facebook_page_insights_daily` ผ่าน cron `facebook-page-insights-sync` โดยใช้ Page token เดิมของ `publish_channels` (มีสิทธิ์ `read_insights` อยู่แล้ว) รองรับเพจเดียวต่อ tenant
