# content-type-badge-display Specification

## Purpose

กำหนดให้ `ContentPlannerCalendar` chip และ `ContentItemList` คอลัมน์ประเภท แสดง "ประเภทเนื้อหา" (บทความ/วีดีโอ ตาม `TYPE_MAP`/`getCanonicalContentType()`) แทนที่การแสดงไอคอนแพลตฟอร์มแยกทีละอัน — พื้นที่ของทั้งสองจุดจำกัดเกินกว่าจะแสดงแพลตฟอร์มได้อย่างมีความหมายเมื่อ content item เลือกไว้หลายแพลตฟอร์ม (สูงสุด 7) รายละเอียดแพลตฟอร์มยังดูได้ครบจาก `ContentCardDialog`/`ContentDetailView` (ดู capability `content-platform-badge-display`)
