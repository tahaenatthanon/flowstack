# cron-job-dispatch Specification

## Purpose

ทำให้งานใน `cron_jobs` ถูกเรียกจริงตามเวลา ไม่ใช่รอคนกดปุ่ม "รันเดี๋ยวนี้" ในหน้าแอดมิน — เก็บตารางเวลาแบบเครื่องอ่านได้ (`cron_expression`) ไว้ในฐานข้อมูล มี entry point เดียว (`api/cron/tick.php`) ที่ตัวตั้งเวลาระดับ OS เรียกทุก 1 นาทีแล้วกระจายงานที่ถึงกำหนด และมีตัวรันงานชุดเดียว (`api/lib/cron-runner.php`) ที่ทั้งตัวตั้งเวลาและปุ่มของแอดมินใช้ร่วมกัน เพื่อไม่ให้พฤติกรรมสองทางแตกต่างกัน
