# quick-create-schedule-date Specification

## Purpose

กำหนดให้ content item ที่สร้างผ่าน Quick Create ได้ `scheduled_date` เป็นวันที่ปัจจุบันโดยอัตโนมัติหลังสร้างสำเร็จ โดยไม่ต้องเพิ่ม date picker ในฟอร์ม เพื่อไม่ให้ item ตกเป็น unscheduled ตั้งแต่ต้น (คู่ขนานกับพฤติกรรมที่ `batch-content-scheduling` มีให้ Batch generation อยู่แล้ว)
