# content-approval-access Specification

## Purpose

กำหนดสิทธิ์ `content_approval` ที่ควบคุมการเห็นแท็บ "รายการอนุมัติ" และการตัดสินอนุมัติคอนเทนต์ (อนุมัติ/ขอแก้ไข/ปฏิเสธ) ทั้งฝั่ง UI และ API — สิทธิ์นี้ต้องมาจาก role assignment จริงเสมอ ไม่ bypass ด้วย `is_admin`/`is_superadmin` เหมือนสิทธิ์เมนูอื่นในระบบ — ที่มา: change `restrict-content-approval-tab`
