# app-clock-timezone Specification

## Purpose

ทำให้นาฬิกาของชั้น API (PHP) กับนาฬิกาของฐานข้อมูล (MariaDB) เป็นค่าเดียวกัน เพื่อไม่ให้เวลาที่เขียนลงคอลัมน์ DATETIME และเวลาที่อ่านกลับมาเทียบ อยู่บนสองกรอบเวลาที่ต่างกัน — ความคลาดแบบนี้ไม่ทำให้อะไรพัง "เสียงดัง" แต่ทำให้ตัวเลขผิดเงียบ ๆ (ตัวตรวจสุขภาพรายงาน `ok` ทั้งที่ cron ตาย, กุญแจที่หมดอายุยังใช้ได้ต่อ, เวลาตอบกลับ Helpdesk ถูกบีบเป็น 0) และครอบคลุมข้อผูกมัดว่าจุดที่ส่งเวลาออกนอกระบบต้องถูกต้องโดยโครงสร้าง ไม่ใช่โดยการตั้งค่า runtime
