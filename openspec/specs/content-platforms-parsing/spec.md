# content-platforms-parsing Specification

## Purpose

กำหนดว่าการ parse ค่า `content_items.platform`/`platforms` (comma-joined string, JSON array string, หรือ array ที่ parse แล้ว) ให้เป็น array แพลตฟอร์มแยกทีละอัน lowercase ไม่ซ้ำ ไม่มีค่าว่าง ต้องมี implementation เดียวที่เป็น canonical ใน `src/lib/contentPlatforms.ts` เท่านั้น ห้ามมีการเขียนตรรกะ parse ซ้ำที่ไฟล์อื่น — เป็น requirement เชิงสถาปัตยกรรม/ป้องกันบั๊ก เพื่อไม่ให้บั๊กคลาสเดียวกัน (เทียบ platform แบบ exact-match กับค่าดิบ) เกิดซ้ำในจุดหนึ่งโดยไม่มีใครรู้หลังถูกแก้ไปแล้วในอีกจุดหนึ่ง (ดู `content-planner-platform-filter-fix`, `content-approval-platform-fix`)
