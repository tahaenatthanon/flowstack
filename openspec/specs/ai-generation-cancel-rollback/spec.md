...
ผู้ใช้กดยกเลิก AI generation ที่กำลังทำงานอยู่ได้จริงในทุกจุดที่เรียก AI (`QuickCreateDialog`, `BatchGenerateDialog`, `ContentPlannerAI` panel, ปุ่ม "AI เขียนให้" ใน `ContentCardDialog`) — UI ตอบสนองทันทีโดยไม่รอ step ที่กำลังรันอยู่เสร็จก่อน และระบบรับประกันว่าจะไม่มีข้อมูลที่ไม่ต้องการหลงเหลืออยู่หลังยกเลิก (ลบ/คืนค่าอัตโนมัติ) แม้ step ปัจจุบันจะรันจนจบไปแล้วในเบื้องหลังก่อนเช็ค flag ทัน (ข้อจำกัดทางสถาปัตยกรรม PHP synchronous ที่ไม่สามารถตัด request กลางคันได้)
