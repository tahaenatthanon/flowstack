# publish-video-readiness-gate Specification

## Purpose

ป้องกันไม่ให้คอนเทนต์วิดีโอที่ยังสร้างไม่เสร็จหรือไฟล์หายไปถูกเผยแพร่ไปยัง Facebook — เพิ่มเช็คใน `final_publish_gate_check()` (`api/lib/publish-dispatch.php`) ซึ่งเป็น gate กลางที่ทั้ง send now, schedule และ cron เรียกร่วมกันอยู่แล้ว จึงครอบคลุมทุกเส้นทางเผยแพร่โดยไม่ต้องเพิ่มจุดเช็คแยก คอนเทนต์ที่ไม่ใช่วิดีโอไม่ได้รับผลกระทบ
