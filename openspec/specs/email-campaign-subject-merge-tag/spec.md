# email-campaign-subject-merge-tag Specification

## Purpose

กำหนดให้ merge tag `{{subject}}` ใน `processMergeTags()` (`api/email-utils.php`) ใช้งานได้จริงเช่นเดียวกับ merge tag อื่น (`{{first_name}}`, `{{company_name}}` ฯลฯ) โดยแทนที่ด้วยหัวข้ออีเมลของแคมเปญที่กำลังส่ง ครอบคลุมทั้ง `body_html` และ `body_text` โดยไม่กระทบพฤติกรรมเดิมของผู้เรียกที่ไม่ได้ส่ง subject มา
