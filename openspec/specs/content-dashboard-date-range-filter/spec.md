# content-dashboard-date-range-filter Specification

## Purpose

กำหนดตัวกรองช่วงวันที่ (Date Range Filter) ของแท็บ "วิเคราะห์" และพารามิเตอร์ `from`/`to` ของ endpoint analytics/result-metrics พร้อมการแยกเมตริกที่ respect ช่วงวันที่ vs snapshot
