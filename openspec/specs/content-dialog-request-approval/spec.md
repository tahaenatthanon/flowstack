# content-dialog-request-approval Specification

## Purpose

Give authors a second, equally direct route into the approval queue: a "ขออนุมัติ" action in the `ContentCardDialog` footer, so work can be submitted straight from the edit dialog without first returning to the detail view. Complements [content-request-approval](../content-request-approval/spec.md), which covers the detail-view action bar.
