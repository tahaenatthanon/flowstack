...
### Requirement: ผู้ใช้แก้ไข `video_prompt` รายฉากผ่าน API ใหม่
ระบบ SHALL มี API action `update-scene` รับ `item_id`, `scene_index`, และ `video_prompt` เพื่ออัปเดตค่าของ scene เดียวใน `article_content.scenes[]` โดยไม่กระทบ scene อื่น
...
- **WHEN** ผู้ใช้กำลังพิมพ์แก้ `video_prompt` แต่ยังไม่กดปุ่ม "บันทึก"
