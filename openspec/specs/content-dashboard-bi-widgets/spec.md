...
แท็บ "วิเคราะห์" SHALL แสดง widget "แนวโน้ม Throughput รายเดือน" เป็นกราฟ (recharts) ย้อนหลัง 12 เดือน มี 4 เส้น: สร้าง / ขออนุมัติ / อนุมัติ / เผยแพร่ โดยแต่ละเมตริกนับในเดือนของ timestamp ตัวเอง

#### Scenario: 4 เส้นต่อเดือนครบ
- **WHEN** แท็บ "วิเคราะห์" โหลดและเรียก `?action=analytics`
...
### Requirement: Widget อัตราสำเร็จการเผยแพร่แยกแพลตฟอร์ม
แท็บ "วิเคราะห์" SHALL แสดง widget "อัตราสำเร็จการเผยแพร่" แยกตามแพลตฟอร์ม (`sent`/`failed`/`pending` จาก `content_publish_queue` join `publish_channels`) พร้อม error ที่พบบ่อยสุดต่อแพลตฟอร์ม

#### Scenario: แยก sent/failed/pending ต่อแพลตฟอร์ม
- **WHEN** แท็บ "วิเคราะห์" โหลด
- **THEN** widget แสดงจำนวน `sent`, `failed`, `pending` แยกตาม `publish_channels.platform`
