<?php
/**
 * Task Hours API — บันทึกชั่วโมงงาน (subtask model)
 *
 * ทุก entry เก็บเป็น subtask (parent_task_id != NULL, is_subtask=1)
 * ชั่วโมงเก็บใน actual_hours + estimated_hours (decimal)
 *
 * GET    ?project_id=  รายการ subtasks ของโปรเจกต์
 * GET    (no params)   รายการทั้งหมดที่ caller เห็นได้
 * GET    ?date_from=&date_to=&user_id=  กรองตามวันที่ / user
 * POST   สร้าง subtask (บันทึกชั่วโมง)
 * PUT    ?id=  แก้ไข
...
function getEntryWithAccess(PDO $db, string $id, string $userId, string $tenantId, bool $isAdmin): array {
    $stmt = $db->prepare('SELECT * FROM tasks WHERE id = ? AND tenant_id = ? AND deleted_at IS NULL');
...
/** Convert a task/subtask row (with joined fields) to task-hours shape */
function rowToEntry(array $t): array {
    return [
        'id'           => $t['id'],
        'is_subtask'   => (int)($t['is_subtask'] ?? 0),
...
$baseSelect = "
    ...
    WHERE s.deleted_at IS NULL AND s.tenant_id = " . $db->quote($tenantId) . "
      AND s.is_subtask = 1
      AND s.parent_task_id IS NOT NULL";
...
if ($method === 'GET') {
    ...
    // Non-admin เห็นเฉพาะงานตัวเอง เสมอ (ไม่ว่าจะกรอง project หรือไม่)
    if (!$isAdmin) {
        $conditions[] = 's.user_id = ?'; $params[] = $userId;
    } elseif ($filterUid) {
        // Admin กรองตาม user ที่เลือก
        $conditions[] = 's.user_id = ?'; $params[] = $filterUid;
    }
...
if ($method === 'POST') {
    ...
    if (empty($parentId)) jsonError('task_id is required — ต้องระบุ task หลักก่อนบันทึกชั่วโมง', 400);
    if (!isValidUUID($parentId)) jsonError('รหัสงาน (task_id) ไม่ถูกต้อง: ' . $parentId, 400);
    ...
    ')->execute([
        ...
        'completed', 'medium', $displayName, $userId,
